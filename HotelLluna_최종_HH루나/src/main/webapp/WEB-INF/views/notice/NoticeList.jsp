    ...
    <div id="wrapper" class="clearfix">
        ...
        <section id="page-title" class="page-title-parallax" style="background-image: url('<c:url value="/resources/images/logo/room.PNG"/>'); padding: 120px 0;" data-stellar-background-ratio="0.3">
            ...
            <div class="container clearfix">
                <FONT SIZE="6" COLOR="white">Hotel Luna | 공지사항 </span></FONT>
                <ol class="breadcrumb">
                </ol>
            </div><br>
			...
			<c:choose>
			  ...
			  <c:when test="${noticeList !=null }" >
			    <c:forEach  var="notice" items="${noticeList }">
			     <tr align="center">
				 <td><c:out value="${notice.b_no}"/> </td>
				  <%-- <td><c:out value="${notice.b_writer}"/> </td>  --%>
				 <td> <a href ="<c:url value='/notice/NoticeContent?b_no=${notice.b_no}'/>">
				   			<c:out value="${notice.b_title}"/> </a></td>
				 <td> <fmt:formatDate value="${notice.b_date}" pattern="yyyy-MM-dd"/></td>
				      <td><c:out value="${notice.b_count}"/> </td>
				</tr>
...
<div class="cls2">
 <c:if test="${totalNotice != null }" >
      <c:choose>
        <c:when test="${totalNotice >100 }">  <!-- 글 개수가 100 초과인경우 -->
	      <c:forEach   var="page" begin="1" end="10" step="1" >
	         <c:if test="${section >1 && page==1 }">
	          <a href="<c:url value='/notice/NoticeList?section=${section-1}&pageNum=${(section-1)*10 +1 }'/>">&nbsp; pre </a>
	         </c:if>
	          <a href="<c:url value='/notice/NoticeList?section=${section}&pageNum=${page}'/>">${(section-1)*10 +page } </a>
	         <c:if test="${page ==10 }">
	          <a href="<c:url value='/notice/NoticeList?section=${section+1}&pageNum=${section*10+1}'/>">&nbsp; next</a>
	         </c:if>
	      </c:forEach>
        </c:when>
        <c:when test="${totalNotice ==100 }" >  <!--등록된 글 개수가 100개인경우  -->
	      <c:forEach   var="page" begin="1" end="10" step="1" >
	        <a class="no-uline"  href="#">${page } </a>
	      </c:forEach>
        </c:when>
        
        <c:when test="${totalNotice< 100 }" >   <!--등록된 글 개수가 100개 미만인 경우  -->
	      <c:forEach   var="page" begin="1" end="${totalNotice/10 +1}" step="1" >
	         <c:choose>
	           <c:when test="${page==pageNum }">
	            <a class="sel-page"  href="<c:url value='/notice/NoticeList?section=${section}&pageNum=${page}'/>">${page } </a>
	          </c:when>
	          <c:otherwise>
	            <a class="no-uline"  href="<c:url value='/notice/NoticeList?section=${section}&pageNum=${page}'/>">${page } </a>
	          </c:otherwise>
	        </c:choose>
	      </c:forEach>
        </c:when>
